...
function IngredientsList({ userid, fromdate, todate }) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:8000/api/ingredients?userid=100&fromdate='2023-01-01'&todate='2023-01-12'`
      )
      .then((response) => {
  ...
  return (
    <div className="row">
      {categories.map((categories) => (
        <div className="card col-sm-4" key={categories}>
          <div className="card-body">
            <h5 className="card-header">{ingredientsCategories[categories]}</h5>
            <div className="card-text">
              <ul>
                {ingredients
                  .filter((i) => i.categories === categories)
                  .map((i) => (
                    <li key={i.id}>
